...
def scan_domain(name, domain):
    ...
    try:
        ...
        try:
            ...
            # subdomains = flare_client.subdomains(_id, domain)
            # print(subdomains)
            # logging.info('Checking domain and subdomains on Shodan....')
            # domains_and_subdomains = [domain] + [x['name'] for x in subdomains['items']]
            # logging.debug(f"Subdomains: {domains_and_subdomains}")

            #new code
            subdomain_names = ['mail', 'mail2', 'www', 'ns2','ns1','blog','localhost','m','ftp','mobile','ns3','smtp','search','api','dev','secure','webmail','admin','img','news','sms','marketing','test','video','www2','media','static','ads','mail2','beta','wap','blogs','download','dns1','www3','origin','shop','forum','chat','www1','image','new','tv','dns','services','music','images','pay','ddrint','conc']
            validated_subdomain = []

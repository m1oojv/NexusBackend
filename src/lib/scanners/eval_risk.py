...
def get_creds_rec(creds_results):
    ...
    rec = {"area": risk_recommendations["creds"]["area"], "details": []}
    ...
    if has_creds_exposed:
        ...
        rec["details"][0]["rec"] = f"Your company credentials have most recently been leaked on <b>{latest_leak.strftime('%d/%m/%Y')}</b> days ago.<br/>" + rec["details"][0]["rec"]
...
def get_vulns_rec(subdomain_results):
    ...
    rec = {"area": risk_recommendations["vulns"]["area"], "details": []}
    if type(subdomain_results) == dict and "error" in subdomain_results.keys():
        # Don't return any rec if there's an error
        return rec

    # Check if any IP in the (sub)domains have an associated vulnerability
    for subdomain in subdomain_results:
        for ip in subdomain["data"]:
            if ip.get("vulns", []):  # There's a CVE, can return the rec already
                logging.debug("CVEs found.")
                rec["details"].append(risk_recommendations["vulns"]["details"][True])
                # rec = risk_recommendations["vulns"]["details"][True]
                return rec

    logging.debug("CVEs not found.")
    # rec = risk_recommendations["vulns"]["details"][False]
    return rec
...
def get_ports_rec(subdomain_results):
    ...
    rec = {"area": risk_recommendations["ports"]["area"], "details": []}
    if type(subdomain_results) == dict and "error" in subdomain_results.keys():
        # Don't return any rec if there's an error
        return rec
...
def get_marketplace_rec(marketplace_results):
    ...
    rec = {"area": risk_recommendations["marketplace"]["area"], "details": []}
...
def get_email_security_rec(email_security_results):
    ...
    rec = {"area": risk_recommendations["email_security"]["area"], "details": []}
    if type(email_security_results) == list or "error" in email_security_results.keys():
        # Don't return any rec if there's an error
        return rec
...
def get_sec_headers_rec(sec_headers_results):
    ...
    rec = {"area": risk_recommendations["security_headers"]["area"], "details": []}

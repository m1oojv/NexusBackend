...
from src.lib.propertieshelper import load_sql_properties
...
def make_connection():
    ...
    # print(script_dir)
    conf = script_dir+'/../config/config.ini'
...
def update_company_score(connection, id, data):
    ...
    clear_score = """
    delete from score_result where company_id = '{id}'
    """.format(id=id)

    deleted_row = update_rows(connection, clear_score)
    
    for row in data:
        update_vendor_query = """
        insert into score_result \
        (id, company_id, control_attribute_id, existence, maturity, effectiveness_adaptability, effectiveness_relevance, effectiveness_timeliness, \
        group_coverage, nist_function, threat_scenario_id, effectiveness_score, tri_score, tri_score_stage) \
        values (gen_random_uuid(), '{uuid}', '{control_id}', '{existence}', '{maturity}', '{effectiveness_adaptability}', '{effectiveness_relevance}', \
        ...
        update_rows(connection, update_vendor_query)
    return True


def control_family_score(connection, control_family, uuid):
    family_scores = []
    ...
    for family in control_family:
        ...
        sql_control_family_query = """
        select c.control_attribute_id, c.maturity, c.effectiveness_relevance, c.effectiveness_timeliness, c.effectiveness_adaptability, c.group_coverage from \
        (select * from score as b where b.control_attribute_id in (SELECT a.id FROM control_attribute as a \
        WHERE a.org_control_family IN ('{controlfamily}')) and b.company_id in ('{uuid}') and b.existence in ('Exists')) as c
        """.format(controlfamily=family, uuid=uuid)

        family_data = retrieve_rows(connection, sql_control_family_query)
        for data in family_data:
            adaptability = convert_score(data[4], "adaptability")
            relevance = convert_score(data[2], "relevance")
            timeliness = convert_score(data[3], "timeliness")
            effectiveness_score = (relevance[0] + timeliness[0] + adaptability[0])/(relevance[1] + timeliness[1] + adaptability[1])
...
# def vendor_ts_control_family_score(connection, control_family, id, vendor):
#     family_scores = []
...
# def control_family_score(connection, control_family):
#     family_scores = []
#
#     for family in control_family:
#         maturity_total = 0
...
# def ts_control_family_score(connection, control_family, id):
#     family_scores = []
